...
#make erdos-renyi 
def make_erdos_graphs_and_pagerank(no_of_nodes, num_of_graphs, folder = 'data_new2', name = 'erdos', avg = 10):
    import networkx as nx
    prob = (avg*no_of_nodes)/(no_of_nodes*(no_of_nodes-1))
    G = nx.erdos_renyi_graph(no_of_nodes, prob, seed=None, directed=True)
    G = nx.DiGraph(G)
    ...
    for i in range(num_of_graphs):        
        ...
        for i in range(100):
            ...
            if not path.isfile(full):
                break
        full_pr = fr"./{folder}/{no_of_nodes}_{name_graph}/{filename}_pagerank.txt"
        ...
        with open(full, 'w') as file:
            ...
            for edge in edges:
                ...
                if edge[2] < 0:
                    ...
                    file.write(f'{edge[0]} {edge[1]} {1+edge[2]}\n')
                    G[edge[0]][edge[1]]['weight'] = 1+edge[2]
                    init_probs[edge[0]] += edge[2]
                    if edge[0] not in init_probs_dic: init_probs_dic[edge[0]] = edge[2]
                    else: init_probs_dic[edge[0]] += edge[2]
                
            for index in range(no_of_nodes):
                file.write(f'{index} {index} {init_probs[index]}\n')
        
        #start = time.time()
        pr = nx.pagerank(G, alpha=1, nstart=init_probs_dic,  max_iter=1000, tol=1e-06)
        with open(full_pr, 'w') as file_pr:
            for pr_score in pr:
                file_pr.write(f'{pr[pr_score]}\n')
        print(f'Saving the pagerank score in file {full_pr}\n')        
    ...
    #print(f'Total time is {totaltime}[secs]')
    #print('\n\n\n')
    #return 0
...
#make scale-free
def make_scale_graphs_and_pagerank(no_of_nodes, num_of_graphs, folder = 'data_new2', name = 'scale', beta = 6):
    import networkx as nx
    G = nx.scale_free_graph(no_of_nodes, alpha=2*(1-beta/10)/3, beta=beta/10, gamma=(1-beta/10)/3, delta_in=0.7, delta_out=0.3)
    G = nx.DiGraph(G)
    print('Graph Characteristics: ', G.number_of_nodes(), G.number_of_edges(), G.number_of_edges()/G.number_of_nodes())
        ...
        with open(full, 'w') as file:
            ...
            for edge in edges:
                ...
                if edge[2] < 0:
                    ...
                    G[edge[0]][edge[1]]['weight'] = 1+edge[2]
                    init_probs[edge[0]] += edge[2]
                    if edge[0] not in init_probs_dic: 
                        init_probs_dic[edge[0]] = edge[2]
                    else: init_probs_dic[edge[0]] += edge[2]
...
if __name__ == "__main__":    
    ...
    for t in range(10):
        for num_of_nodes in [2500, 5000, 7500, 10000, 12500]:
            for beta in [6, 7, 8, 9]:
                if beta == 8:
                    make_scale_graphs_and_pagerank(num_of_nodes, 20, folder, 'scale_beta_'+str((int)(beta))+"_id_"+str(t), beta) 
    ...
    for t in range(10):
        for num_of_nodes in [2500, 5000, 7500, 10000, 12500]:
            for avg in [3,6,9,12]:
                if avg == 6:
                    make_erdos_graphs_and_pagerank(num_of_nodes, 20, folder, 'erdos_avg_'+str((int)(avg))+"_id_"+str(t), avg) 
                elif num_of_nodes == 10000:
                    make_erdos_graphs_and_pagerank(num_of_nodes, 20, folder, 'erdos_avg_'+str((int)(avg))+"_id_"+str(t), avg) 
    sys.exit()
